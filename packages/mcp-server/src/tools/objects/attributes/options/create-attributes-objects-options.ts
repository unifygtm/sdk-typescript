...
export const tool: Tool = {
  name: 'create_attributes_objects_options',
  description:
  ...
  inputSchema: {
    ...
    required: ['object_name', 'attribute_name', 'api_name', 'display_name'],
  },
  annotations: {},
};
...
export const handler = async (client: Unify, args: Record<string, unknown> | undefined) => {
  ...
  return asTextContentResult(
    await maybeFilter(jq_filter, await client.objects.attributes.options.create(attribute_name, body)),
  );
};
