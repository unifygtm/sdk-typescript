...
export interface ClientOptions {
  /**
   * Defaults to process.env['UNIFY_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['UNIFY_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['UNIFY_LOG'] or 'warn' if it isn't set.
   */
...
export class Unify {
   ...
   */
  constructor({
    baseURL = readEnv('UNIFY_BASE_URL'),
    apiKey = readEnv('UNIFY_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.UnifyError(
        'The UNIFY_API_KEY environment variable is missing or empty; either provide it, or instantiate the Unify client with an apiKey option, like new Unify({ apiKey: \'My API Key\' }).'
      );
    }
    ...
    this.timeout = options.timeout ?? Unify.DEFAULT_TIMEOUT /* 1 minute */;
    this.logger = options.logger ?? console;
    const defaultLogLevel = 'warn';
    // Set default logLevel early so that we can log a warning in parseLogLevel.
    this.logLevel = defaultLogLevel;
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('UNIFY_LOG'), 'process.env[\'UNIFY_LOG\']', this) ?? defaultLogLevel;
  ...
  static UnifyError = Errors.UnifyError;
