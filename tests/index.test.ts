...
import { APIPromise } from '@unifygtm/sdk/core/api-promise';

import util from 'node:util';
import Unify from '@unifygtm/sdk';
import { APIUserAbortError } from '@unifygtm/sdk';
...
describe('logging', () => {
  ...
  test('warn when env var level is invalid', async () => {
    ...
    const client = new Unify({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'UNIFY_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    ...
    test('empty env variable', () => {
      ...
      const client = new Unify({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.unifygtm.com')
    });
